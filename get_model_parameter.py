...
parser.add_argument('--Net_Type', default=['M'], choices={'P', 'T', 'C', 'F', 'M'},
                    help="Network Architecture. Convolution (C)"
                         "Transformers (T)")
# Transformers Parameters ------------------------------
parser.add_argument('--permute_data', action="store_false", help='M-FCN class model default permute data')
...
if __name__ == '__main__':
    ...
    problems = [f for f in os.listdir(config['data_path']) if not f.startswith('.')]
    # problems = ['Handwriting']
    problems = ['FaceDetection', 'InsectWingbeat', 'PenDigits', 'SpokenArabicDigits', 'LSST', 'FingerMovements',
                'MotorImagery', 'SelfRegulationSCP1', 'Heartbeat', 'SelfRegulationSCP2', 'PhonemeSpectra',
                'CharacterTrajectories', 'EthanolConcentration', 'HandMovementDirection', 'PEMS-SF', 'RacketSports',
                'Epilepsy', 'JapaneseVowels', 'NATOPS', 'EigenWorms', 'UWaveGestureLibrary', 'Libras',
                'ArticularyWordRecognition', 'BasicMotions', 'DuckDuckGeese', 'Cricket', 'Handwriting', 'ERing',
                'AtrialFibrillation', 'StandWalkJump']
    for model_config in ['O']:
        for problem in problems:  # for loop on the all datasets in "data_dir" directory
