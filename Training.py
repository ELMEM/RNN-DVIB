...
def kl_divergence_multivariate_normal_diag(mu_P, std_P, mu_Q, std_Q):
    """
    计算两个多元正态分布之间的KL散度，假设协方差矩阵是对角矩阵。

    参数:
    mu_P: 分布P的均值向量 (batch_size, dim)
    std_P: 分布P的标准差向量 (batch_size, dim)
    mu_Q: 分布Q的均值向量 (batch_size, dim)
    std_Q: 分布Q的标准差向量 (batch_size, dim)

    返回:
    KL散度 D_{KL}(P || Q) (batch_size,)
    """
    # 计算公式中的每一项
    term1 = (mu_P - mu_Q).pow(2) / std_Q.pow(2)  # (μ_P - μ_Q)^2 / σ_Q^2
    term2 = std_P.pow(2) / std_Q.pow(2)  # σ_P^2 / σ_Q^2
    term3 = 2 * std_Q.log() - 2 * std_P.log()  # 2 * log(σ_Q) - 2 * log(σ_P)
    term4 = 1.0  # 常数项

    # 总和
    kl_divergence = 0.5 * torch.sum(term1 + term2 + term3 - term4, dim=-1)
...
class SupervisedTrainer(BaseTrainer):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            predictions, mu1, std1, mu2, std2,ol = self.model(X.to(self.device))  # xzz: add DVIB loss

            loss = self.loss_module(predictions, targets)  # (batch_size,) loss for each sample in the batch
            KL1 = 0.5 * torch.sum(mu1.pow(2) + std1.pow(2) - 2 * std1.log() - 1, dim=1)
            KL2 = 0.5 * torch.sum(mu2.pow(2) + std2.pow(2) - 2 * std2.log() - 1, dim=1)
            loss = loss + 1e-4 * KL1 + 1e-4 * KL2 + 0.01 * ol
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        # self.epoch_metrics['acc'] = np.sum(predictions == targets) / len(predictions)
        '''
...
def train_runner(config, model, trainer, val_evaluator, test_evaluator, path):
    ...
    for epoch in tqdm(range(start_epoch + 1, epochs + 1), desc='Training Epoch', leave=False):
        ...
        print_str = f'epoch {epoch} Model Test Summary: '
