...
parser.add_argument('--permute_data', type=bool, default=True, help='M-FCN class model permute data')
...
if __name__ == '__main__':
    ...
    model_path = os.path.join(output_dir, config['data_path'])
    file_list = os.listdir(model_path)
    sorted_files_by_time = sorted(file_list, key=lambda x: os.path.getmtime(os.path.join(model_path, x)))
    ...
    for problem in problems:  # for loop on the all datasets in "data_dir" directory
        config['data_dir'] = os.path.join(config['data_path'], 'Multivariate_ts', problem)
        ...
        print(f'total_accuracy is {all_metrics["total_accuracy"]}')
